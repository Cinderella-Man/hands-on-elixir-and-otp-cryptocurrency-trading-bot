...
It comes the time when I need to admit to something that I've learned on the way as I was writing this book and was misused throughout.
...
Instead, we should introduce an additional layer(of abstraction) on top of the 3rd party modules we are using. A typical example of that could be abstracting dealing with an exchange to an behaviour and providing a single implementation(wrapping the `Binance` module):
...
We will start by creating a new behaviour module(inside a new file `/apps/core/lib/core/exchange.ex`) inside the `Core` application:
...
Additionally, we use the `Binance` module to fetch symbol filters inside the `Naive.Strategy` (we actually fetch the whole exchange info and then dig inside the find our filters) - we will create a dedicated struct for those filters:
...
The `fetch_symbol_filters/1` function follows the previously discussed pattern. The `fetch_symbol_filters/2`, on the other hand, is a modified copy of the `merge_filters_into_settings/2` function from the `Naive.Strategy` module.
...
As mentioned, the `BinanceMock` module must implement the `Core.Exchange` behaviour. It will (compile time) guarantee that both `Core.Exchange.Binance` and `BinanceMock` share a common interface that can be used by the `Naive.Strategy`.

Fist we will start by declaring that it actually implements the `Core.Exchange` behaviour:
...
There are a few additional helpers above, and it got a bit long - let's unpack it. First, both `fetch_symbols/0` and `fetch_symbol_filters/1` look very similar to the ones we implemented for the `Core.Exchange.Binance` module. The main difference here is that we are supporting cached exchange info by introducing the `fetch_exchange_info/0` which branches out to either using `Binance` or `get_cached_exchange_info/0` function. The latter was updated to return the raw data instead of the `Binance.ExchangeInfo` struct.

# <= todo: get_order, order_limit_buy, order_limit_sell