...
Don't forget to update all references to the `Binance.Order` with `Exchange.Order` module(and remove it's alias).
...
In the above functions, we simply skipped the fourth argument in the above functions to fulfil the behaviour.
...
Up to this moment, we were using only the `Binance` module. We didn't have an opportunity to work with other modules/exchanges. In fact, we **didn't want to** create an `Exchange` level abstraction. We were happy with using the `Binance` module and we added the behaviour just to be able to use the `Mox` package to mock it inside our tests. All of this feels like a really heavy over-engineering just to be able to test.

Furthermore, as we defined the behaviour, we needed to define the generic structs that we based on the ones from the `Binance` module. We have never seen examples of other structs from other packages, so we took only fields that we are using in our strategy to limit the possibility of missing data from another exchange in the future. The knock-on effect will be that we are already missing valuable data in the database as well as adding any new exchanges in the future will require updates to behaviour, behaviour's existing implementations and most of the code that uses it (like the `Naive.Strategy`).

Additionally, we forwarded `Binance`'s error messages straight to the user of our abstraction(`Naive.Strategy`). We should be converting those to generic errors, but we don't have a clue about other exchanges and the errors they could cause.
...
The first thing that comes to mind would be to look for some "standard" that could be leveraged to build our behaviour/define structs. For cryptocurrency exchanges, that will be "CCXT" - it's actually a open source "library" that is available for Python, JavaScript and PHP.

We could go a step further and look for an Elixir implementation of the CCXT package, and we could find the package named `ccxtex`. It's a wrapper around the JavaScript version of the CCXT package.

Using the above(either the `ccxtex` or `ccxt` as a "standard") instead of trying to figure out our own behaviour based on our limited knowledge about exchanges would most certainly let us avoid continuous updates to our code.

Those updates, coupled with the fact that we would add the behaviour just to be able to test the implementation, puts usage of the `Mox` package in serious doubt.
...
We will stop here, and **revert back** to the source code from the end of the [20th chapter](https://github.com/Cinderella-Man/hands-on-elixir-and-otp-cryptocurrency-trading-bot-source-code/tree/chapter_20). Instead of leaving something that we are definitely not prepared to fix, we will look into alternative ways to test our strategy.
...
That finishes our conversion to hardcoded module names. As we are not basing our module on the configuration anymore, we can remove all redundant configurations from the configuration files:

```{r, engine = 'elixir', eval = FALSE}
# /config/config.exs
config :core,
  logger: Logger,                             # <= remove this
  pubsub_client: Phoenix.PubSub # <= remove this

config :naive,
  binance_client: BinanceMock, # <= remove this
  ...
...
config :naive,
  binance_client: BinanceMock, # <= remove this
  leader: Naive.Leader,               # <= remove this
  repo: Naive.Repo,                     # <= remove this
  ...
```
...
We can use the `expected_order` above to assert that the `Naive.Strategy.execute/3` returned the correct value - the `buy_order` field should hold the same data as the `expected_order` above. We can add the below assertions at the end of the test:
...
That finishes the test implementation, but before we will be able to use the `mimic` module, we need to prepare modules so we can stub them inside tests - here are the new contents of test helper file:
...
$ mix test.unit
...
21:49:39.737 [info] Position (ABC/1675460979732): Placing a BUY order @ 0.800000, quantity: 50.000
.
...
We can see that ExUnit output is now free of any logs.

But what if we would like to assert the logged message? We were able to do that using the `mox` package as we were mocking and asserting inside those mocks.
...
In the above code, we wrapped our call to the `Naive.Strategy.execute/3` into a function that we passed as an argument to the `with_log/1` function. The `with_log/1` function returns a tuple containing the result of the passed function and the generated log message(s). We are then able to assert that the logged message contains the expected value, which strengthens our test. We can now rerun our unit test:
...
That would wrap up the writing the unit tests. But how will this work with running our code locally(or in "production") or running integration tests?

## Swapping back to attributes

We need to have a way to swap implementation between dev and prod environments.

We can't use the `mimic` package to swap the implementation as this is a "running" mode, not some tests.

Please note that most of the applications won't have a need to change the implementation based on the environment as the implementation itself will provide some flags to enable/disable the functionality(like sending emails to newly registered users).
...
# /config/test.exs
config :core,                    # <= remove
  logger: Test.LoggerMock,       # <= remove
  pubsub_client: Test.PubSubMock # <= remove

config :naive,                      # <= remove
  binance_client: Test.BinanceMock, # <= remove
  leader: Test.Naive.LeaderMock,    # <= remove
  repo: Test.Naive.RepoMock         # <= remove
```

It's important to understand that we don't need to configure the `binance_client` inside the test configuration. As we are now utilizing the `mimic` package, we are driving mocking from the test level.
...
...
iex(3)> 
22:57:53.834 [info] Position (ETHUSDT/1675724273832): Placing a BUY order @ 1632.82000000, quantity: 0.12240000
```
...
Source code for this chapter can be found at [Github](https://github.com/Cinderella-Man/hands-on-elixir-and-otp-cryptocurrency-trading-bot-source-code/tree/chapter_21)