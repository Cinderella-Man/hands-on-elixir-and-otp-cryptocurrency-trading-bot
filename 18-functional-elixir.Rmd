...
Across the last 17 chapters, we focused on learning OTP by building a trading system. On the way, we omitted (for a the good reason - a clear focus on OTP) the conversation about functional programming.
...
Let's look at our strategy inside the `Naive.Trader` module. In this section, we will focus on its (`handle_info/2`) callback functions.
...
We can now move to the next callback, similar to the last one we could describe as "fetch the sell order and maybe terminate the trader". We will split it into two callbacks: "fetch the sell order" and "terminate trader".
