...
We could raise the logging level to `error` be done with it. Yes, that would fix all debug/info/warning logs, but we would also miss an opportunity to confirm a few details (depends on what's necessary for our use case):
...
One of the `BinanceMock` module's functions is sending a message to the process that doesn't exist(as we are running our tests without starting the supervision tree[the `--no-start` argument]) to generate a unique id.
...
- we could refactor the `BinanceMock.generate_fake_order/4` to accept id as an argument instead of sending the message internally - this should be a much better solution. Let's give it a shot.
...
- we were able to test a standalone process ignoring all of its dependencies
- we were able to confirm that dependent functions were call and expected values were passed to them
- we were able to create a feedback look where mock was sending a message back to the test, because of which we didn't need to use `sleep`, and that causes massive speed gains
