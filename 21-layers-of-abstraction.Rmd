...
We will stop here, and **revert back** to the source code from the end of the [20th chapter](https://github.com/Cinderella-Man/hands-on-elixir-and-otp-cryptocurrency-trading-bot-source-code/tree/chapter_20). Instead of leaving something that we are definitely not prepared to continue with, we will look into alternative ways to test our strategy.
...
That finishes our conversion to hardcoded module names. As we are not basing our module on the configuration anymore, we can remove all redundant configuration keys from the main `config.exs` file:
