...
Across the last 17 chapters, we focused on learning OTP by building a trading system. On the way, we omitted (for a good reason[clear focus on OTP]) the conversation about functional programming.
...
Note: This section can appear to be a bit "random", but I added it to aid continuity of refactoring steps(refactoring those callbacks later would cause a fair amount of complexity).

Let's look at our strategy inside the `Naive.Trader` module. In this section, we will focus on its callback(`handle_info/2`) functions.

We are looking for functions that do more than "one thing" to split them into multiple functions.
...
We can now move to the next callback, similar to the last one we could describe as "fetch the sell order and maybe terminate the trader". We will split it into two callbacks: "fetch the sell order" and "terminate trader". The code below replaces the 5th `handle_info/2` callback:
...
That finishes our first refactoring round, but I need to admit that our change impacted the behaviour of our strategy. Each time a buy or sell order gets filled, we will fetch that order from Binance, but we *won't* immediately place a sell order nor terminate as it was happening before. Instead, only when another event arrives will the trader place a sell order or terminate.
...
...
  1) test Naive trader full trade(buy + sell) test (NaiveTest)
     ...
     ** (MatchError) no match of right hand side value: [["0.43070000", "BUY", "FILLED"], ["0.43190000", "SELL", "FILLED"]]
...
First, we need to add a single `handle_info/2` callback after the `init/1` function that will pattern match only the fact that the received message contains the `TradeEvent` struct and the state is the correct `State` struct:
...
As for both the race condition events and the "non interesting" events, we are using the `:skip` "decision", we can safely remove this clause as we will implement skipping as the last clause.
...
In the last section, we converted all the `handle_info/2` callback functions into the `generate_decision/2` function. That's excellent progress, but we still have the strategy logic inside the `Naive.Trader` module.

Let's move the `generate_decision/2` function(together with all functions that it depends on, like the `broadcast_order/1` and the `require Logger`) from the `Naive.Trader` module to the `Naive.Strategy` module.
...
Before we update the `Naive.Trader` module to use the `execute/2` function, we need to address another issue that moving the `execute_decision/2` caused. At this moment, all of the clauses return GenServer specific structs. What we really need to return to the trader is an atom indicating should it continue or terminate and the updated state:

```
# /apps/naive/lib/naive/strategy.ex
# last lines inside the `execute_decision/2` functions
{:ok, new_state} # <= previously {:noreply, new_state} (6 times)
:exit # <= previously {:stop, :normal, state}
```
...
# /apps/naive/lib/naive/strategy.ex
  defp execute_decision(
    ...
    logger_info = Map.get(context, :logger_info, &Logger.info/1) # <= fetch from context
    order_limit_buy = Map.get(context, :order_limit_buy, &Binance.order_limit_buy/4) # <= fetch from context
    leader_notify = Map.get(context, :leader_notify, &Leader.notify/2) # <= fetch from context
```
...
And we finally got there - we have come full circle. This is the approach that we previously used inside the `Naive.Trader` module(you can go ahead and add them to the `Naive.Strategy` module):
...
As long as you are using module attributes to be able to inject compile-time dependencies to test your code, there's just *no better way* to do it in Elixir, and now we know why(based on the issues with alternative approaches).
...
def map(%Either.Right(v), fun), do: %Either.Right{result: fun.(v)}
```
...
```
safeDivide(2, 1) # <= returns Right(2)
.then(Either.map(&(safeDivide(&1, 1)))) <= still Right(Right(2))
```
...
In the last section, we discussed wrapping the results in the `Either` structs to be able to map, flatMap on them regardless of the function result. What if we could apply the same principle to avoid executing side effects at all?

That's the basic idea behind all the category theory related abstractions like the infamous IO Monad.

I won't go into a huge amount of details but we can think about it as every time we are calling a *special* `map` or `flatMap`, instead of executing anything, it would just wrap whatever was passed to it inside another function and return it like:
...
In statically typed languages, there's an argument that instead of a function of function etc., we could have a typed object which would indicate what actions can be performed on that future result. This is very often praised as a compile-time guarantee of side effect full code.

In the Elixir without strong typing and with a massive impact on how the code is written and how easy it is to understand, there's just *no* reason to use those concepts beyond toy programs. The resulting function would be an untestable blob without introspection support from the BEAM VM.

## Final thoughts

Every programming language needs to provide tools to handle common concerns like errors or dependency injection.

Elixir provides excellent tools to handle both of those concerns using the `with` statement and modules' attributes.

Without a type system, there's no practical reason why anybody would introduce category theory-based abstraction like Monads. The resulting code will be complicated to deal with, and as in the case of many other programming languages like Ocaml or Clojure, the pragmatic way is to execute side effects.
...
Source code for this chapter can be found at [Github](https://github.com/Cinderella-Man/create-a-cryptocurrency-trading-bot-in-elixir-source-code/tree/chapter_18)