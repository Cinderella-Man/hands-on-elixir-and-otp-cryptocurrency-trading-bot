...
...
  ...
  alias Hedgehog.Repo
  alias Hedgehog.Streaming.Binance.Schema.Settings
...
...
defmodule Hedgehog.Streaming.Settings do
  ...
  schema "streaming_settings" do
    field(:symbol, :string)
    field(:streaming_status, SettingsStatusEnum)
...
defenum(Hedgehog.Streaming.SettingsStatusEnum, :streaming_status, [:on, :off])
```
...
With the above changes, the reintegration of the `Streamer` app is finished, and we can test that everything works up to this point:
