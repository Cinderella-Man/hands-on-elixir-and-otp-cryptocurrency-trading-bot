...
As a challenge, you could update the `naive` and the `streamer` application to use the `Registry` and remove `Core.ServiceSupervisor` module as it was superseded by the above solution - here's the [link to PR](https://github.com/frathon/create-a-cryptocurrency-trading-bot-in-elixir-source-code/pull/7)(pull request) that sums up the required changes.
