...
In the previous chapter, we gave the `streamer` app proper supervision, start/stop functionality,
and autostart capabilities. Now it's time to do the same for the `naive` trading app - but with an important twist.

Trading is different from streaming.
When we stop a streamer, we just close a WebSocket connection - no big deal.
But when we stop a trader, it might be in the middle of a trade cycle:
it bought some asset and is waiting for the price to rise before selling.
If we abruptly terminate it, we're left holding a position we didn't intend to keep.

That's why we'll implement four operations in this chapter:

- **start** - begin trading on a symbol (same pattern as streaming)
- **stop** - immediately terminate trading (useful for emergencies)  
- **shutdown** - gracefully stop trading by letting all active traders complete their current trade cycles before terminating
- **autostart** - resume trading on previously enabled symbols when the app restarts

The shutdown feature is the key addition here.
It gives us a controlled way to wind down trading without getting stuck with unwanted positions. Let's build it!
...
In the 9th chapter, we've introduced the Postgres database inside the `naive` application together with the settings per symbol.

In this chapter, we will progress forward to provide additional trading functionality that will be similar to
the functionality implemented in the last chapter for the `streamer` application:

* **stop trading** - as the `Naive.SymbolSupervisor` processes are registered with names that can be easily reverse engineered, we should be able to utilize the `Process.whereis/1` function to retrieve the PIDs and instruct the `Naive.DynamicSymbolSupervisor` to terminate those child processes. Again, we need to put that logic somewhere so we will implement the `Naive.DynamicSymbolSupervisor` as a full module using the `DynamicSupervisor` behavior.
...
Both implementation and helper functions are almost the same as the ones inside the `streamer` application. It could be tempting to abstract some of the logic away but remember that we should treat all applications in our umbrella project as standalone services that should not share any code if possible(we broke that rule for the `TradeEvent` struct from the `streamer` app but we could easily just make a lib with that struct that would be shared between two applications). I would shy away from sharing any business logic between applications in the umbrella project.
...
Next, we will create a `shutdown_trading/1` function inside the `Naive.DynamicSymbolSupervisor` module
where we will check if there is any trading going on for that symbol(the same as for start/stop),
and in case of trading happening we will inform the `Naive.Leader` process handling that symbol that settings have been updated:
...
* whenever the `Naive.Trader` process will finish the trade cycle, `Naive.Leader` process should *not* start a new one, as well as check if that was the last trader process - if so, it needs to call the `Naive.stop_trading/1` function with its symbol to terminate the whole supervision tree for that symbol
...
As visible in the above code, whenever the `Naive.Trader` process will finish the trade cycle,
the `Naive.Leader` process will check if it can find a record of that trader in its state (no changes here).
We will modify the callback so the leader process will check the `settings.status`.
In the `shutdown` status it checks whether it was the last trader in the `traders` list,
to terminate the whole tree at that time(using the `Naive.stop_trading/1` function).
...
We can now test the `shutdown_trading/1` functionality:
...
As we can see from the logs above, our `naive` strategy grew from 1 to 5 `Naive.Trader` processes running in parallel,
then we called the `shutdown_trading/1` function.
In the `shutdown` status, the `Naive.Leader` process ignored `rebuy` notifications and
wasn't starting any new `Naive.Trader` processes as the old ones were finishing.
At the moment when the last `Naive.Trader` process finished the trade cycle,
the `Naive.Leader` called `stop_trading/1` on its symbol, terminating the whole supervision tree for that symbol.

**The naive trading app now has full lifecycle control!** We've implemented:

- Start/stop trading with database-backed status persistence
- Graceful shutdown that lets traders complete their cycles
- Autostart functionality that resumes trading after app restart
- A `rest_for_one` supervision strategy ensuring autostart runs when needed

The shutdown feature is particularly important for a real trading system. It means we can safely wind down trading without being left with open positions or incomplete trades.

**So, What's Next?**

If you've been following along, you've probably noticed that the `Naive.DynamicSymbolSupervisor` and `Streamer.DynamicStreamerSupervisor` modules look remarkably similar. We've essentially copy-pasted the same supervision patterns between them.

In the next chapter, we'll address this duplication by creating a `Core.ServiceSupervisor` module that abstracts the common logic. We'll use Elixir macros to generate the boilerplate code, letting us define supervisors with minimal repetition while keeping each application independent.
