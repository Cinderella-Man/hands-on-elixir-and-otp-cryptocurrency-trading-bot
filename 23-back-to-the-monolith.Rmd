...
defmodule Hedgehog.Streamer.Settings do # <= updated
  use Ecto.Schema

  alias Hedgehog.Streamer.SettingsStatusEnum # <= updated

  @primary_key {:id, :binary_id, autogenerate: true}

  schema "streamer_settings" do # <= updated
    field(:symbol, :string)
    field(:status, SettingsStatusEnum) # <= updated
...
defenum(Hedgehog.Streamer.SettingsStatusEnum, :status, [:on, :off]) # <= updated
```
...
defmodule Hedgehog.Repo.Migrations.CreateStreamerSettings do # <= updated
  use Ecto.Migration

  alias Hedgehog.Streamer.SettingsStatusEnum # <= updated

  def change do
    SettingsStatusEnum.create_type() # <= updated

    create table(:streamer_settings, primary_key: false) do # <= updated
      add(:id, :uuid, primary_key: true)
      add(:symbol, :text, null: false)
      add(:status, SettingsStatusEnum.type(), default: "off", null: false) # <= updated

      timestamps()
    end

    create(unique_index(:streamer_settings, [:symbol])) # <= updated
  end
end
```
