...
defmodule Hedgehog.Strategy.Naive.Formula do
  ...
  defp broadcast_order(%Binance.Order{} = order) do
    Phoenix.PubSub.broadcast( # <= updated
      Hedgehog.PubSub,
      "ORDERS:#{order.symbol}",
      order
    )
...
In line with the other Enum modules' changes, we will update the file name from `/lib/hedgehog/strategy/naive/trading_status_enum.ex` to `/lib/hedgehog/strategy/naive/settings_status_enum.ex`.
...
The final file that we copied across from the `naive` umbrella app's is `application.ex`, where we will look into the supervised `children` list. It was supervising the `Naive.Repo`, which we don't need any more and `Naive.Supervisor`, which we renamed and need to put into our new main `application.ex` file:
...
First, we will move two migration files from the `/apps/naive/priv/repo/migrations/` to `/priv/repo/migrations`. We will rename the `20210202223209_create_settings.exs` to `20210202223209_create_naive_strategy_settings.exs` where we will update the module name and all references to modules and table name:
...
Next, we will rename the `20210205232303_update_trading_status.exs` to `20210205232303_update_naive_strategy_settings_status.exs` where we will update the module name and all references to modules:
...
...
Logger.info("Inserting default streamer settings for symbols") # <= updated
...
# updated, `on_conflict` added
{count, nil} = Repo.insert_all(StreamerSettings, maps, on_conflict: :nothing)

Logger.info("Inserted streamer settings for #{count} symbols") # <= updated
...
Next, we will rename the `/test/hedgehog/strategy/naive/strategy_test.exs` to `/test/hedgehog/strategy/naive/formula_test.exs` and update its module name and aliases:
...
...
[info] Position (XRPUSDT/1715703392316): Placing a BUY order @ 0.50650000, quantity: 394.86000000
[info] Position (XRPUSDT/1715704251578): The BUY order is now partially filled
[info] Position (XRPUSDT/1715704251578): The BUY order is now filled. Placing a SELL order @ 0.50690000, quantity: 394.86000000
...
[info] Position (XRPUSDT/1715704342038): Placing a BUY order @ 0.50680000, quantity: 394.63000000
```
...
`schema/trade_event.ex` -> `exchange/trade_event.ex`
...
We will copy three migration files across from the `data_warehouse` app (the `/apps/data_warehouse/priv/repo/migrations/` directory) to the new Phoenix application (the `priv/repo/migrations/` directory).

In the case of the `20210227230123_create_subscriber_settings.exs`, we need to rename it to `20210227230123_create_collector_settings.exs`.
...
We will copy the `/apps/indicator/lib/indicator/ohlc` directory and the `/apps/indicator/lib/indicator/ohlc.ex` file into it.
...
A quick look at the `application.ex` module of the `indicator` app will tell us that we need to add a dynamic supervisor to the supervision tree of the new app (we referred to it already as `Hedgehog.Data.Aggregator.DynamicWorkerSupervisor` inside the `Hedgehog.Data.Aggregator` as we were updating it):
...
...
[debug] Broadcasting OHLC: %Hedgehog.Data.Aggregator.Ohlc{symbol: "XRPUSDT", start_time: 1717173780, duration: 1, open: "0.51350000", high: "0.51380000", low: "0.51350000", close: "0.51370000"}
```
