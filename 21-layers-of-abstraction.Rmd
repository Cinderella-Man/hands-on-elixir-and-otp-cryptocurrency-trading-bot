...
There are a few additional helpers above, and it got a bit long - let's unpack it. First, both  the `fetch_symbols/0` and `fetch_symbol_filters/1` look very similar to the ones we implemented for the `Core.Exchange.Binance` module. The main difference here is that we are supporting cached exchange info by introducing the `fetch_exchange_info/0`, which branches out to either using the `Binance` module or the `get_cached_exchange_info/0` function. The latter was updated to return the raw data instead of the `Binance.ExchangeInfo` struct.
...
```{r, engine = 'elixir', eval = FALSE}
  ...
  defp convert_order_to_event(%Exchange.Order{} = order, time) do
    ...
    %TradeEvent{
      ...
      price: order.price,
      quantity: order.quantity,  # <= updated mapping
      buyer_order_id: order.id, # <= updated mapping
      seller_order_id: order.id, # <= updated mapping
      trade_time: time - 1,
