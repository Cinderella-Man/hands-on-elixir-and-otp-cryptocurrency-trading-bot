...
Besides the above, we are now relying on the generic structs, so we need to update all reference to the `Binance.OrderResponse` module with the `Exchange.Order` (including updating all field names) - for example:
...
As now we will deal only with the `Exchange.Order` structs instead a pair of `Binance.OrderResponse` and `Binance.Order`, we can simplify the existing two clauses of `broadcast_order/1` into a single one:
...
# /config/prod.exs
config :naive,
  exchange_client: Core.Exchange.Binance # <= key updated
